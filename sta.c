...
{
    struct station_pos *p_temp_sta, *p_new_sta;
    struct monitor_pos *p_temp_mon, *p_new_mon;
    p_temp_sta = p_start_sta;

    int csere_sta = 0, csere_mon = 0;

    while (p_temp_sta)		//Kliens keresése
	{		if(DEBUG) printf("  akt_sta: " MACSTR " ", MAC2STR(p_temp_sta->station_address));
	if (memcmp(adat->station, p_temp_sta->station_address, MACLEN) == 0)		//Kliens megvan
	    {		if(DEBUG) printf("talalt_sta\n");

	    if (p_record->enabled && p_temp_sta->record)
		{		if (DEBUG) printf("Recording data\n");
		FILE *record_file;
		char filename_rec[40];
		char dirname[40];
		sprintf(dirname, "positions/%s/" MACSTR, p_record->nev, MAC2STR(adat->station));
		struct stat st;
		if (stat(dirname,&st))
		    {
		    mkdir(dirname, 0755);
	    ...
	    p_temp_mon = p_temp_sta->monitor;
	    while (p_temp_mon)		//Monitor keresése
		{		if(DEBUG) printf("    akt_mon: %s ", inet_ntoa(p_temp_mon->monitor_address));
		if (memcmp(adat->monitor, p_temp_mon->serial, MACLEN) == 0)		//Monitor megvan, adatok frissítése
		    {		if(DEBUG) printf("talalt_mon\n");
		    
		    p_temp_mon->signal		= adat->signal;
		    p_temp_mon->monitor_address = kuldo.sin_addr;
		    p_temp_mon->time_rcv	= time(NULL);
		    p_temp_sta->channel		= adat->channel;
		    p_temp_sta->time_last	= time(NULL);

		    if(DEBUG) printf("1; MON: %s MAC: " MACSTR " SIG: %i CH: %i\n", inet_ntoa(p_temp_mon->monitor_address), MAC2STR(p_temp_sta->station_address), p_temp_mon->signal, p_temp_sta->channel);

		    csere_mon = 1;
		    break;
		    }
		else
		    {		if(DEBUG) printf("nem_talalt_mon\n");
		    p_temp_mon = p_temp_mon->next_mon;
		    }
		}		if(DEBUG) printf("mon_keres_vege\n");
	    if (csere_mon == 0)		//Monitor nincs meg, új monitor hozzáadás
		{		if(DEBUG) printf("uj_mon\n");
		...
		p_new_mon->next_mon		= p_temp_sta->monitor;

		p_temp_sta->monitor		= p_new_mon;
		p_temp_sta->channel		= adat->channel;
		p_temp_sta->time_last		= time(NULL);

		if(DEBUG) printf("2; MON: %s MAC: " MACSTR " SIG: %i CH: %i\n", inet_ntoa(p_new_mon->monitor_address), MAC2STR(p_temp_sta->station_address), p_new_mon->signal, p_temp_sta->channel);
		csere_sta = 1;
		}
	    break;
	    }
	    else
		{	if(DEBUG) printf("nem_talalt_sta\n");
		p_temp_sta = p_temp_sta->next_sta;
		}
	}	if(DEBUG) printf("sta_keres_vege\n");
    if (csere_sta == 0 && csere_mon == 0)		//Kliens nincs meg, új kliens hozzáadás
	{	if(DEBUG) printf("uj_sta\n");
	p_new_sta = (struct station_pos *) malloc(sizeof(struct station_pos));
	p_new_mon = (struct monitor_pos *) malloc(sizeof(struct monitor_pos));
	p_new_sta->position = (struct position_pos *) malloc(sizeof(struct position_pos));
	p_new_sta->pos_xy = (struct position_pos *) malloc(sizeof(struct position_pos));
	...
	p_new_sta->channel		= adat->channel;
	p_new_sta->time_last		= time(NULL);
	p_new_sta->record		= 0;
	p_new_sta->next_sta		= NULL;
//	p_new_sta->position		= NULL;
//	p_new_sta->pos_xy		= NULL;
	memcpy(p_new_sta->station_address, adat->station, MACLEN);
	...
	if (p_start_sta == NULL)
	    ...
	    {
	    p_temp_sta = p_start_sta;
	    while (p_temp_sta->next_sta)
		{
		p_temp_sta = p_temp_sta->next_sta;
		}
	    p_temp_sta->next_sta = p_new_sta;
	    }
	
	if(DEBUG) printf("3; MON: %s MAC: " MACSTR " SIG: %i CH: %i\n", inet_ntoa(p_new_mon->monitor_address), MAC2STR(p_new_sta->station_address), p_new_mon->signal, p_new_sta->channel);
	}
}
